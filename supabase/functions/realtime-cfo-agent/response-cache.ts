  ...
  try {
    ...
    if (targets === null || targets.length === 0) {
      ...
      if (targets === null) {
        ...
        if (count > 0) console.log(`[${reqId}] Cache INVALIDATED ALL: ${count} entries (unknown write op)`);
        return count;
      }
      return 0;
    }

    // Targeted: delete cache entries whose path matches any invalidation target
    let totalDeleted = 0;
    ...
    if (totalDeleted > 0) {
      console.log(`[${reqId}] Cache TARGETED INVALIDATION: ${totalDeleted} entries (targets: ${targets.join(", ")})`);
    }
    return totalDeleted;
  } catch (e) {
