...
export async function logFeedback(
  ...
  try {
    ...
    if (error) console.error(`[${reqId}] Feedback log error:`, error.message);
    else console.log(`[${reqId}] Feedback logged: path=${entry.route_path}, tools=${entry.tools_used?.length || 0}`);
  } catch (e) {
